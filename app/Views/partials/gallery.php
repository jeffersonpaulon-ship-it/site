        ...
        <?php if (!empty($galleryItems) && is_array($galleryItems)): ?>
            <?php foreach ($galleryItems as $case): ?>
                <?php 
                    ...
                    // Mapeamento direto das colunas da tabela `cases`
                    $tituloDoEvento = $case['title'] ?? 'Case de Sucesso';
                    $nomeDoCliente  = $case['client'] ?? $tituloDoEvento;
                    $linkSlug       = $case['slug'] ?? '';
                    $arquivoImagem  = $case['image_site'] ?? '';
                ?>
