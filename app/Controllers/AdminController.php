...
class AdminController extends BaseController
{
    ...
    {
        $caseModel = new CaseModel();
        $cases = $caseModel->orderBy('id', 'DESC')->findAll();
    ...
    private array $caseImageFields = [
        'image_site', 'image_banner', 'featured_image',
        'carousel_image_1', 'carousel_image_2', 'carousel_image_3',
        'carousel_image_4', 'carousel_image_5', 'carousel_image_6',
    ];

    private function caseTextFields(): array
    {
        return [
    ...
    // Exibe o formulário de criação de um case
    public function createCases()
    {
        return view('admin/case_create');
    }

    // Armazena um novo case
    public function storeCases()
    {
        ...
        $data = [];
        foreach ($this->caseTextFields() as $field) {
            $data[$field] = $this->request->getPost($field);
        }
        $data['slug'] = url_title($data['slug'], '-', true);

        foreach ($this->caseImageFields as $field) {
            $file = $this->request->getFile($field);
            if ($file && $file->isValid() && !$file->hasMoved()) {
                $newName = $file->getRandomName();
                $file->move(FCPATH . 'assets/images/cases', $newName);
                $data[$field] = $newName;
            }
        }
        ...
        return redirect()->to('/admin/cases')->with('success', 'Case criado com sucesso!');
    }

    // Exibe o formulário de edição de um case
    public function editCases($id)
    {
        ...
        if (!$case) {
            return redirect()->to('/admin/cases')->with('error', 'Case não encontrado.');
        }

        return view('admin/case_edit', ['case' => $case]);
    }

    // Atualiza um case existente
    public function updateCases($id)
    {
        $caseModel = new CaseModel();
        ...
        return redirect()->to('/admin/cases')->with('success', 'Case atualizado com sucesso!');
    }
    ...
    public function deleteCases($id)
    {
        ...
        return redirect()->to('/admin/cases')->with('success', 'Case excluído com sucesso!');
    }
